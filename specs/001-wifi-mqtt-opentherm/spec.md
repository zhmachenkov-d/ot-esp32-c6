...
- Q: How must non-ID1 writable command failures be observable? → A: **Same pattern as ID 1** — publish an explicit signal on `…/ot/<N>/rejection` for non-success outcomes (`rejected_range`, `rejected_failsafe`, `ot_failed`); reflected state stays last accepted (or empty if never accepted).
...
- Q: Which OT exchanges count toward boiler-link consecutive failures? → A: **Keepalive/status only** (the ≥1 Hz Status cadence)—not slow-/fast-tier catalog reads.
...
- **Boiler-link health**: Dedicated HA entity reflecting OpenTherm master↔slave link healthy vs unhealthy, distinct from MQTT device availability. Unhealthy after **3 consecutive failed keepalive/status** exchanges; healthy after one successful keepalive/status exchange (tiered catalog reads do not count).
- **Writable command**: A Home Assistant control change mapped to an OpenTherm master command for a boiler-supported writable Data ID (including ID 1 Control setpoint via `WRITE-DATA`, and Status/CH-enable via master flags on the ID 0 Status exchange—never `WRITE-DATA(id=0)`). Non-success outcomes publish on `…/ot/<N>/rejection`.
