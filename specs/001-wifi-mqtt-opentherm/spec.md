...
- Q: When should the dedicated boiler-link health entity report unhealthy versus healthy? → A: Unhealthy after N consecutive failed OT exchanges (or equivalent short failure window); healthy after successful exchange(s)
...
- **Boiler-link health**: Dedicated HA entity reflecting OpenTherm master↔slave link healthy vs unhealthy, distinct from MQTT device availability. Unhealthy after documented consecutive OT exchange failures (default 3); healthy after successful exchange(s).
