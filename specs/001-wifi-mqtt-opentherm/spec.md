...
- Q: Besides MQTT device availability, what boiler “status” must Home Assistant see in the initial entity set? → A: MQTT availability + explicit boiler-link health entity; no extra flame/fault/CH-active sensors in v1 *(superseded: when ID 0 is available, Status flag projections and other supported readable Data IDs appear per FR-002 / data-model)*
...
- **Fail-safe state**: Gateway mode while Wi‑Fi or MQTT is down; continues OT keepalive, holds the last commanded CH setpoint, refuses new remote Data ID writes until the link recovers; MQTT availability is `offline` while active (writes still allowed during the pre-entry timer only).
