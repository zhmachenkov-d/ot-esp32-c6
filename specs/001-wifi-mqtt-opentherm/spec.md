...
- Out-of-range CH setpoint write: reflected setpoint stays at last accepted value; explicit rejection signal is published on `…/ot/1/rejection`; boiler is not written. Other writable IDs use the same `…/ot/<N>/rejection` pattern for non-success outcomes.
...
- **FR-013**: CH Control setpoint (Data ID 1) commands outside the effective min/max MUST be **rejected** (not clamped): the gateway MUST NOT write an out-of-range value to the boiler, MUST retain the last accepted setpoint on the reflected entity, and MUST provide **observable feedback** as both (1) unchanged last-accepted reflected value and (2) an **explicit rejection signal** published on `…/ot/1/rejection` (see contracts)—no silent ignore, silent clamp, or HA-only display of the rejected out-of-range value. **Every boiler-supported writable Data ID** (including ID 1) MUST publish non-success on `…/ot/<N>/rejection`. **Internal** command `outcome` values: `accepted` \| `rejected_range` \| `rejected_failsafe` \| `ot_failed`. **Wire** JSON `reason` on that topic: `out_of_range` when outcome is `rejected_range`; `rejected_failsafe` and `ot_failed` unchanged—see `contracts/mqtt-ha-discovery.md` / data-model `SetpointRejectionSignal`. A dedicated HA diagnostic/`event` entity MAY additionally mirror that signal but is **not** required for v1. Effective **max** MUST prefer the boiler-reported CH setpoint upper-limit Data ID when available (commonly **ID 57** / Max CH water setpoint, or the boiler’s equivalent supported max-limit ID). Effective **min** MUST use SoftAP-configured / firmware-default min in v1 (**no standard OpenTherm CH Control setpoint lower-limit Data ID is assumed**); a boiler min-limit ID MAY override SoftAP min only when that ID is catalog-available and explicitly listed in `firmware/tests/host/fixtures/` bound/write-safe fixtures. When no boiler max-limit ID is available, max MUST also use SoftAP-configured values seeded from firmware defaults. Boiler limit IDs that are available remain readable entities per FR-002 and, when used for bounds, are inputs to rejection—not display-only.
...
- **Setpoint bounds**: Effective CH water setpoint min/max for Data ID 1 — prefer boiler max-limit Data ID (commonly ID 57) for max; **v1 min defaults to SoftAP/firmware** (no standard OT lower-limit ID assumed); otherwise SoftAP-persisted operator values seeded from firmware defaults. Bound/write-safe fixture overrides live under `firmware/tests/host/fixtures/`.
- **Fail-safe state**: Gateway mode after the **10 000 ms** entry timer expires while Wi‑Fi or MQTT remains down; continues OT keepalive, holds the last commanded CH setpoint, refuses new remote Data ID writes until the link recovers; **application** MQTT availability stays `online` during the entry timer and becomes `offline` when active (Option A).
...
- Fail-safe **Option A**: application MQTT availability stays `online` during the 10 s entry timer; becomes `offline` when fail-safe is active (FR-006).
