...
2. **Given** the operator sets a CH water setpoint outside the allowed range, **When** the command is published, **Then** the gateway **rejects** it (does not write an out-of-range value to the boiler), keeps the last accepted setpoint on the reflected entity, and publishes an **explicit rejection** on the ID 1 rejection status topic—not a silently clamped value and not HA-only display of the out-of-range number.
...
- **FR-012**: The operator MUST be able to identify gateway online/offline (MQTT availability) distinctly from boiler link healthy/unhealthy via a dedicated boiler-link health entity (not only via sensor/climate unavailable). Boiler-link MUST report **unhealthy** after a documented consecutive OpenTherm exchange failure threshold (default: **3 consecutive failed** keepalive/status or in-scope exchanges at the ≥1 Hz cadence), and MUST report **healthy** again after **one successful** keepalive/status exchange—not on a single isolated failure before the unhealthy threshold, and not solely by mirroring Data ID entity unavailable.
- **FR-013**: CH Control setpoint (Data ID 1) commands outside the effective min/max MUST be **rejected** (not clamped): the gateway MUST NOT write an out-of-range value to the boiler, MUST retain the last accepted setpoint on the reflected entity, and MUST provide **observable feedback** as both (1) unchanged last-accepted reflected value and (2) an **explicit rejection signal** published on the MQTT status topic for ID 1 rejection (see contracts)—no silent ignore, silent clamp, or HA-only display of the rejected out-of-range value. A dedicated HA diagnostic/`event` entity MAY additionally mirror that signal but is **not** required for v1. Effective **max** MUST prefer the boiler-reported CH setpoint upper-limit Data ID when available (commonly **ID 57** / Max CH water setpoint, or the boiler’s equivalent supported max-limit ID). Effective **min** MUST use SoftAP-configured / firmware-default min in v1 (**no standard OpenTherm CH Control setpoint lower-limit Data ID is assumed**); a boiler min-limit ID MAY override SoftAP min only when that ID is catalog-available and explicitly listed in bound/write-safe fixtures. When no boiler max-limit ID is available, max MUST also use SoftAP-configured values seeded from firmware defaults. Boiler limit IDs that are available remain readable entities per FR-002 and, when used for bounds, are inputs to rejection—not display-only.
...
- **Writable command**: A Home Assistant control change mapped to an OpenTherm master command for a boiler-supported writable Data ID (including ID 1 Control setpoint via `WRITE-DATA`, and Status/CH-enable via master flags on the ID 0 Status exchange—never `WRITE-DATA(id=0)`).
- **Setpoint bounds**: Effective CH water setpoint min/max for Data ID 1 — prefer boiler max-limit Data ID (commonly ID 57) for max; **v1 min defaults to SoftAP/firmware** (no standard OT lower-limit ID assumed); otherwise SoftAP-persisted operator values seeded from firmware defaults.
- **Fail-safe state**: Gateway mode after the **10 000 ms** entry timer expires while Wi‑Fi or MQTT remains down; continues OT keepalive, holds the last commanded CH setpoint, refuses new remote Data ID writes until the link recovers; MQTT availability is `offline` while active (writes still allowed during the pre-entry timer only).
