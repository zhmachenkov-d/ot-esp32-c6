...
- [ ] T023 [US1] Integrate SoftAP → STA → MQTT → catalog validate → discovery into `firmware/main/main.c` end-to-end path; after first successful OT+MQTT link, run `idf.py size` (or equivalent) and note app `.bin` vs soft budgets (≤1.5 MiB / warn if >1.2 MiB) so size regressions surface before polish T043
